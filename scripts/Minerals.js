        ...
        if (currentFacilityId) {
            ...
            let currentMinerals = []
                ...
                if (response.ok) {
                    ...
                    currentMinerals.forEach(mineral => html += `<input class="mineralRadio" type="radio" id=${mineral.id} name="mineral" value="${mineral.id}"/><label for=${mineral.id}>${mineral.name}</label>`)
                }
        }
    }
    return html
